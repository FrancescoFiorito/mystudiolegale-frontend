...
export default function ResetPassword() {
  ...
  const submit = async () => {
    ...
    setLoading(true);
    try {
      await api.post("/auth/reset-password", { email, token, new_password: pw });
      ...
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={{ flexGrow: 1 }} keyboardShouldPersistTaps="handled">
          <View style={s.hero}>
            ...
            <Text style={[s.title, { color: t.onBrand }]}>Nuova password</Text>
            <Text style={[s.subtitle, { color: t.onBrand, opacity: 0.8 }]}>Scegli la tua nuova password</Text>
          </View>
